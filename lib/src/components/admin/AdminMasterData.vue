...
// v2.0.0: destinations carry a `type` -- PROJECT (default, subject to
// per-purchaser assignment) or OFFICE/MARKETING/OTHER (open to any
// Business Trip purchaser, no assignment required).
const projectForm = reactive<{ code: string; name: string; description: string; defaultCurrencyId: number; type: DestinationType }>({ code: '', name: '', description: '', defaultCurrencyId: 0, type: 'PROJECT' })
...
const destinationTypes: DestinationType[] = ['PROJECT', 'OFFICE', 'MARKETING', 'OTHER']
...
async function addProject(): Promise<void> {
	...
	try {
		...
		Object.assign(projectForm, { code: '', name: '', description: '', defaultCurrencyId: currencies.value.find(c => c.isDefault)?.id ?? 0, type: 'PROJECT' })
...
async function saveMembers(): Promise<void> {
	...
	for (const role of ['PURCHASER','ACCOUNTANT'] as const) {
...
<template>
	<section class="master-data">
		...
		<article class="panel">
			<header><div><p class="eyebrow">{{ t('pettycash', 'Organization') }}</p><h2>{{ t('pettycash', 'Destinations (Projects, Office, Marketing…)') }}</h2></div></header>
			<p class="muted">{{ t('pettycash', 'PROJECT destinations require assigning specific purchasers below. OFFICE/MARKETING/OTHER destinations are open to any Nextcloud user submitting a Business Trip Cost List -- no purchaser assignment needed for those.') }}</p>
			<div class="form-row project-form">
				<input v-model="projectForm.code" :placeholder="t('pettycash','Code')">
				<input v-model="projectForm.name" :placeholder="t('pettycash','Name')">
				<select v-model="projectForm.type"><option v-for="dt in destinationTypes" :key="dt" :value="dt">{{ dt }}</option></select>
				<select v-model.number="projectForm.defaultCurrencyId"><option v-for="currency in currencies.filter(c => c.active)" :key="currency.id" :value="currency.id">{{ currency.code }}</option></select>
				<input v-model="projectForm.description" :placeholder="t('pettycash','Description')">
				<NcButton variant="primary" @click="addProject">{{ t('pettycash','Create destination') }}</NcButton>
			</div>
			<div class="project-grid"><button v-for="project in projects" :key="project.uuid" class="project-card" :class="{selected: selectedProjectUuid === project.uuid}" @click="selectedProjectUuid = project.uuid"><strong>{{ project.code }}</strong><span>{{ project.name }}</span><small>{{ project.type }} · {{ project.active ? t('pettycash','Active') : t('pettycash','Inactive') }}</small></button></div>
			<div v-if="selectedProject" class="project-detail">
				<div class="detail-title"><div><h3>{{ selectedProject.name }}</h3><p>{{ selectedProject.code }} · {{ selectedProject.type }}</p></div><NcButton @click="toggleProject(selectedProject)">{{ selectedProject.active ? t('pettycash','Archive') : t('pettycash','Activate') }}</NcButton></div>

				<template v-if="selectedProject.type === 'PROJECT'">
					<h4>{{ t('pettycash','Purchaser & accountant assignment') }}</h4>
					<p class="muted">{{ t('pettycash', 'Enter Nextcloud user IDs separated by commas. Note: Manager 1 and Manager 2 are now assigned per-purchaser and per-destination on the Assignments panel above, not here -- these fields control who may purchase against this destination and who accounts for it.') }}</p>
					<div class="role-grid"><label>{{ t('pettycash','Purchasers') }}<input v-model="memberText.PURCHASER"></label><label>{{ t('pettycash','Accountants') }}<input v-model="memberText.ACCOUNTANT"></label></div>
					<NcButton variant="primary" @click="saveMembers">{{ t('pettycash','Save assignment') }}</NcButton>
				</template>
				<p v-else class="muted">{{ t('pettycash', 'This destination type is open to any Nextcloud user via a Business Trip Cost List -- no purchaser assignment applies.') }}</p>
