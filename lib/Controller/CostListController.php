...
final class CostListController extends BaseApiController {
    ...
    // v2.0.0: no projectUuid -- Cost Lists are scoped to the purchaser,
    // not a project. listType defaults to REGULAR; currencyId is
    // optional and falls back to the configured default currency.
    #[NoAdminRequired] #[ApiRoute(verb:'POST',url:'/api/v1/lists')]
    public function create(int $jalaliYear,int $jalaliMonth,string $listType='REGULAR',?int $currencyId=null):DataResponse{return $this->respond(fn()=>$this->service->create($jalaliYear,$jalaliMonth,$listType,$currencyId),Http::STATUS_CREATED);}
    ...
    #[NoAdminRequired] #[ApiRoute(verb:'DELETE',url:'/api/v1/lists/{uuid}')]
